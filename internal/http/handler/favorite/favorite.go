...
func (h *FavoriteHandler) AddWorkspace(c *gin.Context) {
	if err := h.favorites.Add(c.Request.Context(), common.CurrentUserID(c), "workspace", c.Param("workspaceId")); err != nil {
...
func (h *FavoriteHandler) RemoveWorkspace(c *gin.Context) {
	if err := h.favorites.Remove(c.Request.Context(), common.CurrentUserID(c), "workspace", c.Param("workspaceId")); err != nil {
		common.RespondError(c, err)
		return
	}
...
func (h *FavoriteHandler) AddBoard(c *gin.Context) {
	if err := h.favorites.Add(c.Request.Context(), common.CurrentUserID(c), "board", c.Param("boardId")); err != nil {
		common.RespondError(c, err)
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "favorited"})
...
func (h *FavoriteHandler) RemoveBoard(c *gin.Context) {
	if err := h.favorites.Remove(c.Request.Context(), common.CurrentUserID(c), "board", c.Param("boardId")); err != nil {
		common.RespondError(c, err)
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "unfavorited"})
