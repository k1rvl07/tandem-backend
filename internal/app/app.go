...
func (a *App) BuildRouter(ctx context.Context) (*gin.Engine, error) {
	...
	workspaceService := workspace.NewService(workspace.Deps{Workspaces: workspaceRepo, Users: userRepo, Favorites: favoriteRepo, Boards: boardRepo, Columns: columnRepo, Tasks: taskRepo, Files: fileService, Hub: a.hub, Cache: a.redis, Logger: a.logger})
	workspaceHandler := hworkspace.NewWorkspaceHandler(workspaceService)
	boardService := board.NewService(board.Deps{Boards: boardRepo, Columns: columnRepo, Tasks: taskRepo, Workspaces: workspaceRepo, Users: userRepo, Favorites: favoriteRepo, Files: fileService, Hub: a.hub, Cache: a.redis})
	boardHandler := hboard.NewBoardHandler(boardService)
	taskService := task.NewService(task.Deps{Tasks: taskRepo, Columns: columnRepo, Boards: boardRepo, Workspaces: workspaceRepo, Users: userRepo, Files: fileService, Hub: a.hub, Cache: a.redis})
	taskHandler := htask.NewTaskHandler(taskService)
	attachmentService := attachment.NewService(attachment.Deps{Attachments: attachmentRepo, Tasks: taskRepo, Columns: columnRepo, Boards: boardRepo, Workspaces: workspaceRepo, Files: fileService, Hub: a.hub, Cache: a.redis})
	attachmentHandler := hattachment.NewAttachmentHandler(attachmentService)
	favoriteService := favorite.NewService(favorite.Deps{Favorites: favoriteRepo, Workspaces: workspaceRepo, Boards: boardRepo, Hub: a.hub, Cache: a.redis})
	favoriteHandler := hfavorite.NewFavoriteHandler(favoriteService)
	treeService := tree.NewService(tree.Deps{Workspaces: workspaceRepo, Boards: boardRepo, Columns: columnRepo, Tasks: taskRepo, Users: userRepo, Favorites: favoriteRepo, Cache: a.redis})
